function beginLoad() {
  /* Inserts iframe used for product loading in the document head */
...
function createTable() {
  /* Create the cart */
  if (!cart.isEmpty()) { // Check if the cart is not empty
    ...
    document.getElementById('ccontent').style.display = 'none'; // Hide the cart
    document.getElementById('nocart').style.display = 'inherit'; // Show the no cart message
  }
}

function changeAndUpdate(add_or_remove, rownumber) {
  /* Adds and removes the products from the cart and updates the table */
...
function getProductInformation(callback) {
  /* Store the product information into _products_array_
     Bounces between this and its callback until there are no longer anymore products to load
  */
...
function getProductInformation_cont(result, products) {
  /* Callback for getProductInformatino() */
...
function populateData() {
  /* Fill in data from _products_array_ */
  for (var i = 0; i < cart.countProducts(); i++) {
...
function getTotalCartPrice() {
  // Get the total price of the cart (exc shipping)
  try {
    var price = 0;
    for (var i = 0; i < cart.countProducts(); i++) {
